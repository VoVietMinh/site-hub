...
export async function generateKeywords(
  ...
  const tone =
    opts.tone ||
    (site?.default_tone as string | undefined) ||
    config.articles.defaultTone;
...
export async function buildArticle(
  ...
  if (!claimed)
    throw Object.assign(
      new Error('Article is not PENDING or already claimed'),
      { status: 409 },
    );
...
async function _buildPipeline(articleId: number): Promise<void> {
  ...
  function gen(
    prompt: string,
    opts?: Parameters<typeof llm.generate>[1],
  ): Promise<unknown> {
    const prefix =
      language !== 'Vietnamese'
  ...
  async function save(
    fields: Parameters<typeof repo.updateArticle>[1],
  ): Promise<void> {
  ...
  try {
    ...
    if (!article.outline) {
      ...
      if (article.content_html) {
        await step('outline');
        outlineText = (await gen(
          render(PROMPT['outlineNoResearch']!, {
            KEYWORD: article.keyword,
        ...
        outlineText = researchText
          ? ((await gen(
              render(PROMPT['outlineWithResearch']!, {
                KEYWORD: article.keyword,
                OUTLINE_COUNT: article.outline_count,
    ...
    const needSections =
      ...
      !(article.content_html as string).includes('<h2>');
    ...
    if (needSections) {
      ...
      for (let i = 0; i < sections.length; i += CHUNK) {
        await step(
          `sections:${i + 1}-${Math.min(i + CHUNK, sections.length)}/${sections.length}`,
        );
    ...
    let imageUrls: Array<string | null> = existingImages.map(
      (img) => img.wp_media_url ?? null,
    );
      ...
      try {
        ...
        if (translated) {
          ...
          await logRepo.write({
            ...
            category: 'cse',
            message:
              'IMAGE.KEYWORD_TRANSLATE original="' +
              article!.keyword +
              '" english="' +
              englishKeyword +
              '"',
    ...
    async function generateMetadata(): Promise<void> {
      ...
      )) as { mainKeyword: string; metaDescription: string; tags: string[] };
    ...
    if (needSections) {
      ...
      const finalImageData = (await repo.listImagesForArticle(articleId))
        ...
        .map((img) =>
          img.wp_media_url
            ? { url: img.wp_media_url as string, alt: kw }
            : null,
        );
      ...
      if (!html.includes('<h2>') || html.length < 1000) {
        await fail(
          `Content too short or missing <h2> sections (length: ${html.length})`,
        );
        return;
      }
    ...
    if (site && article.publish_mode === 'immediate') {
      ...
      await repo.updateArticle(articleId, {
        status: 'QUEUED' as Article['status'],
      });
    }
  } catch (err) {
...
export function stitchHtml(
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    if (img?.url) {
      ...
      html +=
        '<div class="image-section"><img src="' +
        img.url +
        '" alt="' +
        a +
        '" title="' +
...
export async function publishArticle(
  ...
  const targetSiteId = (
    opts.siteId !== null ? opts.siteId : article.site_id
  ) as number | null;
  if (!targetSiteId)
    throw Object.assign(
      new Error('No site assigned — please select a publish site first'),
      { status: 409 },
    );
  ...
  const targetCategoryId =
    opts.categoryId !== null
      ? opts.categoryId
      : (article.category_id as number | null);
  ...
  if (isFirstPublish) {
    await repo.updateArticle(articleId, {
      status: 'PUBLISHING' as Article['status'],
      error_message: null,
    });
  }
  ...
  try {
    const tagNames: string[] = Array.isArray(article.tags)
      ...
      : article.tags
        ? JSON.parse(article.tags as string)
        : [];
    ...
    if (isScheduled)
      payload['date_gmt'] = new Date(article.scheduled_at as string)
        .toISOString()
        .replace(/\.\d{3}Z$/, '');
    ...
    if (article.main_keyword || article.meta_description) {
      ...
      if (article.main_keyword)
        (payload['meta'] as Record<string, unknown>)['_yoast_wpseo_focuskw'] =
          article.main_keyword;
      if (article.meta_description)
        (payload['meta'] as Record<string, unknown>)['_yoast_wpseo_metadesc'] =
...
export async function retryArticle(
  ...
  if (article.status !== 'FAILED')
    throw Object.assign(new Error('Only FAILED articles can be retried'), {
      status: 409,
    });
  await repo.updateArticle(articleId, {
