...
const config = {
    ...
    database: {
        url: process.env['DATABASE_URL'] ?? 'postgres://panel_user:panelpass123@localhost:5432/panel_db',
    },
    superAdmin: {
    ...
    articles: {
        defaultOutlineCount: parseInt(process.env['ARTICLE_DEFAULT_OUTLINE_COUNT'] ?? '9', 10) || 9,
        defaultTone: process.env['ARTICLE_DEFAULT_TONE'] ?? 'natural, humanize',
        sectionWordsMin: parseInt(process.env['ARTICLE_SECTION_WORDS_MIN'] ?? '500', 10) || 500,
        sectionWordsMax: parseInt(process.env['ARTICLE_SECTION_WORDS_MAX'] ?? '700', 10) || 700,
        tagCreateDelayMs: parseInt(process.env['TAG_CREATE_DELAY_MS'] ?? '3000', 10) || 3000,
    },
